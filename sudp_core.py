        ...
        for name, df in sheets.items():
            ...
            if name == "연료사용량":
                ...
                if nrows > 0 and ncols > 0:
                    r1, r2 = 2, nrows + 1
                    if split_col >= 0:
                        left_last = max(split_col - 1, 4)
                        ws.conditional_format(r1, 0, r2, min(4, ncols-1), {"type":"no_blanks","format":fmt_int})
                        if left_last >= 5:
                            ws.conditional_format(r1, 5, r2, left_last, {"type":"no_blanks","format":fmt_num})
                        if split_col + 6 <= ncols - 1:
                            ws.conditional_format(r1, split_col + 6, r2, ncols-1, {"type":"no_blanks","format":fmt_num})
                    else:
                        ws.conditional_format(r1, 0, r2, min(4, ncols-1), {"type":"no_blanks","format":fmt_int})
                        if ncols > 5:
                            ws.conditional_format(r1, 5, r2, ncols-1, {"type":"no_blanks","format":fmt_num})
                continue
